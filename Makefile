...
IMG ?= ghcr.io/kluctl/flux-kluctl-controller:latest
...
deploy: manifests
	cd config/manager && kustomize edit set image ghcr.io/kluctl/flux-kluctl-controller=${IMG}
	kustomize build config/default | kubectl apply -f -

# Deploy controller dev image in the configured Kubernetes cluster in ~/.kube/config
dev-deploy: manifests
	mkdir -p config/dev && cp config/default/* config/dev
	cd config/dev && kustomize edit set image ghcr.io/kluctl/flux-kluctl-controller=${IMG}
...
api-docs: gen-crd-api-reference-docs
	$(GEN_CRD_API_REFERENCE_DOCS) -v=4 -api-dir=./api/v1alpha1 -config=./hack/api-docs/config.json -template-dir=./hack/api-docs/template -out-file=./docs/api/flux-kluctl-controller.md
...
docker-build: manager
	cp $(BUILD_DIR)/bin/manager ./manager
	docker buildx build \
...
docker-deploy:
	kubectl -n flux-system set image deployment/flux-kluctl-controller manager=${IMG}
